...
def get_weakest_concepts(user_id: str, limit: int = 3) -> list[dict]:
    """
    ...
    try:
        ...
        # to distinguish "weak" from "new/untouched"
        mastery_response = (
            supabase_admin.table("user_concept_mastery")
            .select("*")
            .eq("user_id", user_id)
            .gt("times_tested", 0)  # Only concepts we've actually tried
            .order("mastery_score", desc=False)  # Ascending = lowest first
            .limit(limit)
            .execute()
        )
