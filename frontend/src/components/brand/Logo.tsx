"use client";

import { motion } from "framer-motion";
import { cn } from "@/lib/utils";

interface LogoProps {
  className?: string;
  size?: "sm" | "md" | "lg" | "xl";
  showText?: boolean;
  animated?: boolean;
}

// Mascot head path
const MASCOT_HEAD_PATH = "M1006.98 666.764C1120.11 308.446 1605.94 269.517 1779.76 594.804C1802.71 637.748 1816.13 684.065 1824.77 731.813C1826.52 741.481 1829.72 756.332 1830.16 765.696C1833.11 782.774 1835.1 800.916 1836.51 818.198C1842.42 890.518 1842.36 970.382 1798.59 1032.32C1798.2 1032.87 1797.81 1033.41 1797.41 1033.95C1783.84 1057.39 1749.48 1087.21 1726.06 1100.44C1715.11 1107.17 1703.73 1113.18 1691.98 1118.41C1685.21 1121.53 1679.28 1124.34 1672.2 1126.81C1657.38 1131.95 1616.79 1145.2 1601.02 1146.66C1585.06 1150.55 1519.51 1161.7 1505.37 1160.09C1494.68 1162.3 1452.96 1162.86 1440.81 1162.93C1378.61 1163.3 1309.19 1163.45 1247.82 1152.6C1242.91 1152.35 1231.25 1149.76 1225.98 1148.71C1140.81 1131.73 1064.42 1103.2 1015.02 1027.81C997.348 999.553 982.989 961.143 979.721 927.912C971.64 845.742 983.157 745.842 1006.98 666.764Z";

const LEFT_EYE = { cx: 1250, cy: 750, r: 40 };
const RIGHT_EYE = { cx: 1566, cy: 750, r: 40 };

// Tightly crop to the mascot head
const MASCOT_VIEWBOX = "940 230 920 960";

// Mascot as the "A" - mascot is BIGGER than text now
const sizes = {
  sm: { mascot: "w-5 h-5", text: "text-base", gap: "" },
  md: { mascot: "w-7 h-7", text: "text-lg", gap: "" },
  lg: { mascot: "w-8 h-8", text: "text-xl", gap: "" },
  xl: { mascot: "w-12 h-12", text: "text-2xl", gap: "" },
};

export function Logo({ className, size = "md", showText = true, animated = true }: LogoProps) {
  const sizeClasses = sizes[size];

  const MascotSvg = (
    <motion.svg
      viewBox={MASCOT_VIEWBOX}
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={cn("shrink-0", sizeClasses.mascot)}
      preserveAspectRatio="xMidYMid meet"
      whileHover={animated ? { scale: 1.1, rotate: [-2, 2, -2, 0] } : {}}
      transition={{ duration: 0.5 }}
    >
      <path
        d={MASCOT_HEAD_PATH}
        className="fill-[var(--brand-secondary)]"
      />
      <circle
        cx={LEFT_EYE.cx}
        cy={LEFT_EYE.cy}
        r={LEFT_EYE.r}
        className="fill-[var(--surface-dark)]"
      />
      <circle
        cx={RIGHT_EYE.cx}
        cy={RIGHT_EYE.cy}
        r={RIGHT_EYE.r}
        className="fill-[var(--surface-dark)]"
      />
    </motion.svg>
  );

  if (!showText) {
    return animated ? (
      <motion.div
        className={className}
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
      >
        {MascotSvg}
      </motion.div>
    ) : (
      <div className={className}>{MascotSvg}</div>
    );
  }

  const content = (
    <div className={cn("flex items-center", sizeClasses.gap, className)}>
      {MascotSvg}
      <span
        className={cn(
          "font-bold tracking-tight text-[var(--text-primary)] font-heading -ml-1.5",
          sizeClasses.text
        )}
      >
        utoCoach
      </span>
    </div>
  );

  if (animated) {
    return (
      <motion.div
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        className="cursor-pointer"
      >
        {content}
      </motion.div>
    );
  }

  return content;
}

// Full logo for hero/footer - mascot as "A" with wordmark
export function LogoFull({
  className,
  size = "lg",
  centered = false
}: {
  className?: string;
  size?: "md" | "lg" | "xl";
  centered?: boolean;
}) {
  const sizeClasses = {
    md: { mascot: "w-9 h-9", text: "text-3xl", gap: "" },
    lg: { mascot: "w-12 h-12", text: "text-4xl", gap: "" },
    xl: { mascot: "w-18 h-18", text: "text-5xl", gap: "" },
  }[size];

  return (
    <motion.div
      className={cn(
        "flex items-center",
        centered && "justify-center",
        className
      )}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6 }}
    >
      <div className={cn("flex items-center", sizeClasses.gap)}>
        {/* Mascot as "A" - bigger than text */}
        <motion.svg
          viewBox={MASCOT_VIEWBOX}
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={sizeClasses.mascot}
          preserveAspectRatio="xMidYMid meet"
          animate={{ y: [-2, 2, -2] }}
          transition={{ duration: 4, ease: "easeInOut", repeat: Infinity }}
          whileHover={{ scale: 1.08, rotate: [-3, 3, -3, 0] }}
        >
          <path
            d={MASCOT_HEAD_PATH}
            className="fill-[var(--brand-secondary)]"
          />
          <circle
            cx={LEFT_EYE.cx}
            cy={LEFT_EYE.cy}
            r={LEFT_EYE.r}
            className="fill-[var(--surface-dark)]"
          />
          <circle
            cx={RIGHT_EYE.cx}
            cy={RIGHT_EYE.cy}
            r={RIGHT_EYE.r}
            className="fill-[var(--surface-dark)]"
          />
        </motion.svg>

        {/* Wordmark */}
        <span
          className={cn(
            "font-extrabold tracking-tight text-[var(--text-primary)] font-heading -ml-2.5",
            sizeClasses.text
          )}
        >
          utoCoach
        </span>
      </div>
    </motion.div>
  );
}
