...
- **Smoke test:** **passed** (2026-05-10) for trace visibility — user observed the parent `quiz.generate_questions` span in the UI with `environment=production`, `release=<git sha>`, no instrumentation exceptions. ~~generation child tagged `model=kimi-k2.6`~~ — **correction (2026-07-08, #72):** this did not reflect the code at the time. There was never a separate generation child span, and nothing set a `model` field on any observation — `call_kimi`/`call_openai` were deliberately left uninstrumented (commit `26747c6`). #71/#72 fixed this: `quiz.generate_questions`/`quiz.evaluate_free_text` (still the same top-level `@observe(as_type="generation")` span, no child) now carry `model` + `usage_details` via `update_current_generation(...)` per Kimi/OpenAI-fallback branch, plus `session_id`/`user_id` via `propagate_attributes(...)` so traces are filterable on the Sessions/Users tabs.
...
- **2026-05-30 update:** plan rewritten to "eval-first, $0, local-only." See `tasks/todo.md#eval-first-roadmap-2026-05-30`. Phase 1.7 step 1 (self-hosted Langfuse) replaced by Langfuse Cloud free tier. Golden set sized down to 30/doc. Ragas runner scaffolded at `backend/evals/` (see below).
...
2. **Retrieval quality** — Langfuse Cloud → Scores tab → filter by trace name `ragas_eval`, or by tags `eval`, `ragas`, and the document id. `context_recall` and `context_precision` are the headline retrieval-quality numbers. Re-run `python -m evals.run_ragas` weekly and watch the trend.
...
```
Check `evals/results/*.csv` for the per-row breakdown, and the `ragas_eval` trace in Langfuse Cloud for the aggregate.
